...
def back_tracking(N, M, pos, m_slop, p_slop, m=0, empty=0):
    if M == m:
        return m - empty
    cnt = place = 0
    if pos[m]:
        for x, y in pos[m]:
            if not m_slop[x + y] and not p_slop[x - y]:
                ...
                cnt = max(back_tracking(N, M, pos, m_slop, p_slop, m + 1, empty), cnt)
                m_slop[x + y] = p_slop[x - y] = False
            else:
                place += 1
        if len(pos[m]) == place:
            cnt = max(back_tracking(N, M, pos, m_slop, p_slop, m + 1, empty + 1), cnt)
    else:
        cnt = max(back_tracking(N, M, pos, m_slop, p_slop, m + 1, empty + 1), cnt)
    return cnt
...
def solution(N, board):
    ...
    for x, y in [(x, y) for x in range(N) for y in range(N)]:
        if board[y][x] == "1":
            pos[x + y].append((x, y))
    return back_tracking(N, 2 * N - 1, pos, m_slop, p_slop)
